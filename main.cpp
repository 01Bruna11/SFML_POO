...
struct Bubble {
	...
	void draw(sf::RenderWindow& window) {
		...
		pencil.write(std::string(1, letter), x - 8.8 + Bubble::radius, y, Bubble::radius + 9.5, sf::Color::White);
	} 
};
...
struct Board {
	...
	void draw() {
		...
		pencil.write("Size: " + std::to_string(this->bubbles.size(), 40, 30, 20, sf::Color::Black);
		for (Bubble& bubble : bubbles) {
			bubble.draw(window);
		}
	}
